  ...
  return (
    ...
    <header className="flex justify-between items-center h-20 bg-header-color text-white px-5 lg:px-16">
      ...
      <nav className="hidden lg:flex space-x-6 ">
        ...
        <Link key={item.id}
            className=" text-2xl p-2 hover:text-yellow-400 focus:text-yellow-400 border-t-4 border-b-4 border-transparent hover:border-t-4 focus:border-t-4 hover:border-b-4 focus:border-b-4 hover:border-yellow-400 focus:border-yellow-400 transition duration-500"
            to={item.link}>
            {item.name}
        </Link>
        )}
      ...
      <button
        onClick={toggleMenu}
        className="lg:hidden max-w-lg:flex flex items-center outline-none "
      ...
      >
        <img
          src={hamburgerLogo}
          alt="Menu"
          className={`opacity-80 hover:opacity-100 focus:opacity-100 transition-transform transform  ${isOpen ? 'rotate-90' : ''}`}
